...
create table namestart_e (
[name] varchar(20)
)


insert namestart_e
select Person.Person_FirstName 
from Person.Person
where Person.Person_FirstName like 'e%'

--baska bir columndan e ile baslayanlarý kopyaladýk
...
--default deger verýlmemýs ýse null deger verir.
--

INSERT Book.Publisher
DEFAULT VALUES
--publisher id'e identity tanmladýgýmýz için o 1'den baþlar, publisher_name'de default deger verýlmedýgý ýcýn bize Null deger verir
--person.person da hata verýr cunku primary key'e identity tanýmlanmamýs ve default deger tanýmlanmamýs
--bu kodu kullanýrsak primary key'e null atayacagý ýcýn býze hata verýr
...
--personfirstname'deki tüm isimleri default_name yapar
...
--ssn 78962212466 olan sütünlarýn personfirstname'i Can yaptýk

UPDATE Person.Person_2
SET Person_FirstName = 'Ali'
WHERE Person_LastName = 'Yýlmaz'
--soyadý yýlmaz olanlarýnýn adýný ali yap dedýk
...
--Personlastname'i null olan degerleri sil dedik
...
-- hata verir çünkü author'da primer key yok. önce primer key belirlemek lazým
...
--hata verir çünkü authorid'de null degerler var. Önce not null yapmamýz gerek.


ALTER TABLE Book.Author ALTER COLUMN Author_ID INT NOT NULL
--not nul yaptýk þimdi primery key yapýcaz üst koddan. sonra da foreign keyleri belirleyecegýz
